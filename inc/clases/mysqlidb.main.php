...
// Variable which holds last statement error
    ...
    //ahora es public porque es empleada asi en paginacion, lo arreglaremos
    protected $_mysqli;
    public $public_mysqli;
    ...
    public function __construct(){
        ...
        if($this->db_port == NULL){     $port = ini_get('mysqli.default_port');
        }else{                      $port = $db_port;                     }
        if($this->db_name == NULL){ $database = $a[0];
        }else{                      $database = $db_name;             }

        $this->_mysqli = new mysqli(self::$host, self::$db_user, self::$db_pass, $database, $port)
...
//habra dos constructores, el primero recibira y asignara parametros 
// y el segundo se conectara a la tabla decidida por el primero(y con sus datos) 
    ...
    //crearemos un metodo publico para acceder a $_mysqli que es protected
    public function publicar_mysqli(){
        $this->public_mysqli = $this->_mysqli;
        return $this->public_mysqli;
    }
